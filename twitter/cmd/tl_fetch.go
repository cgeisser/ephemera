...
func TimelineFetch(cmd *cobra.Command, args []string) {
	fetchAll(func(v url.Values) ([]anaconda.Tweet, error) {
		api := twitterAPI()
...
func fetchAll(apiCall func(url.Values) ([]anaconda.Tweet, error)) {
...
func fetch(apiCall func(url.Values) ([]anaconda.Tweet, error),
	b *leveldb.Batch, v url.Values) (int, error) {
	tl, err := apiCall(v)
	if err != nil {
		return 0, err
	}
